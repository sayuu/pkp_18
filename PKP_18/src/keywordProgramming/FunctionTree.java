...
public class FunctionTree implements Comparable<FunctionTree>{
	 ...
	 */
	public FunctionTree(Function f, List<Word> input_words, HashMap<String, Integer> numOfSameWordsIn){
		...
		String flg_common = console_test.ConsoleTest3.para.common_subsequence;
		
		if(flg_common == null){
			IPreferenceStore store = Activator.getDefault().getPreferenceStore();
			flg_common = store.getString(PreferenceInitializer.COMMON_SUBSEQUENCE);
		}
		...
		if(flg_common.equals(PreferenceInitializer.COMMON_SUBSEQUENCE_OFF))
			...
			//キーワードとラベルの部分一致でもＯＫ.
			e_vec = ExplanationVector.calcExplanationVector2(input_words, numOfSameWordsIn, this);
	}
